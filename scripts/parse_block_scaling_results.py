...
if __name__ == "__main__":
    ...
    parser.add_argument("-p","--prefix",required=False,help="prefix of input filenames")
    ...
    for root, dirnames, filenames in os.walk(args.src):
        for filename in fnmatch.filter(filenames, args.prefix+'*.txt'):
    ...
    for name in files:
        ...
        f.readline()                                     #data size: 131072  (may be incorrect because this was simply a parameter to the script!)
        ...
        size=int(f.readline().split(' ')[1])             #total_data_size: 131072
