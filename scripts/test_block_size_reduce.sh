...
time $HOME/code/spark/bin/spark-submit --master spark://${1}:7077 ${app} --generate ${nblocks} ${block_sz} --dst ${dst} -n ${3} -p ${nparts} >> /projects/visualization/cam/experiment-logs/test_block_size_reduce-${nodes}nodes-${size}mb.out 2>&1 
